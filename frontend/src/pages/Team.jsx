    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mt-1">Members across all your projects</p>
        </div>
        <span className="flex items-center gap-2 text-sm font-medium text-[var(--text-muted)] bg-[var(--bg-tertiary)] px-3 py-1.5 rounded-full border border-[var(--border-color)]">
      ...
      {allMembers.length === 0 ? (
        <div className="py-20 text-center border border-dashed border-[var(--border-color)] rounded-lg">
          <Users className="w-12 h-12 mx-auto mb-3 text-gray-300" />
          <p className="text-[var(--text-muted)] font-medium">No team members found</p>
          <p className="text-sm text-gray-400 mt-1">Create a project and invite team members to see them here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {allMembers.map(member => (
            <div key={member.id} className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg p-5 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="mt-4 pt-3 border-t border-[var(--border-light)]">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {member.projects.slice(0, 3).map((p, i) => (
                      <span key={i} className="text-xs bg-[var(--bg-tertiary)] text-[var(--text-muted)] px-2 py-0.5 rounded border border-[var(--border-color)]">
                        {p}
                      </span>
                    ))}
